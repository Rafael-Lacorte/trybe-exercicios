...
function MyProvider({ children }) {
  ...
  const updateItem = (itemTypeList, indexPresentInList, item) => {
    setOrderList(orderList[itemTypeList][indexPresentInList] = item);
  };

  const removeItem = (itemTypeList, indexPresentInList, item) => {
    setOrderList(orderList[itemTypeList][indexPresentInList]
  ...
  const handleChange = ({ target }, itemName, itemPrice, itemType) => {
    ...
    const isPresentInList = orderList[itemType].includes(newItem.id);
